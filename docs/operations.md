...
This document is the durable operating baseline for the customer-ready MVP founding launch targeted from 15 September 2026, with 15 October 2026 as the latest planned paid opening for the main January-deadline cohort. The founder owns each procedure until a named delegate is recorded. Privacy, terms, safeguarding, complaints, accessibility, refunds, and retention require specialist review before the production launch gate can pass; the date never overrides a failed gate.
...
The command creates two disposable confirmed synthetic accounts, obtains direct anonymous/authenticated PostgREST sessions, uses the staging database connection to inspect every migration through `20260827202316`, RLS, grants, triggers and constraints, and removes all synthetic rows in a verified `finally` path. It must never use production values. Only redacted project identifiers may be printed. Record the project/environment, migration versions, timestamp, operator, pass/fail result for migration/RLS, cross-user reads, browser writes, raw catalogue, cross-object relationships, entitlement concurrency, service RPCs, audit/payment integrity, and cleanup outcome. A failed or unrun suite blocks production; static migration-string tests are supplementary only. The threat register and same-origin route assumptions are documented in [`threat-model.md`](threat-model.md).
...
The founder owns the source register. A source can create commercial published records only after an administrator records an active, auditable permission attestation in `/admin/catalogue`: the applicable API terms, open-licence conditions, or direct permission basis plus a brief confirmation note. No document upload or written reference is required. A configured API key alone is not an attestation. Automated imports create candidates or pending revisions; human review is always required for published material facts. During pre-launch preparation only, an isolated non-production project may use the exact `CATALOGUE_UNAPPROVED_DRAFT_IMPORT_ACK` acknowledgement to retain source observations as unpublished drafts while an attestation is pending. This mode is refused when the configured project matches `SUPABASE_PRODUCTION_PROJECT_REF`; it cannot publish records, expose them publicly, or make readiness pass.

| Source | Owner and evidence | Access and permitted-use position | Attribution | Cadence and failure policy | Approval status |
| --- | --- | --- | --- | --- | --- |
| Find an Apprenticeship Display Vacancy Advert API v2 | [API catalogue](https://www.api.gov.uk/tas/display-vacancy-advert-api/) | API key and commercial/display terms must be confirmed with the source owner. Pagination/rate limits are treated as contractual integration parameters, not inferred from this repository. | Record any wording supplied by the source owner before publication. | Six-hour incremental full snapshot; no closure after incomplete/failed run; alert and investigate failed runs. | **Blocked** until an admin records an active `api-terms-confirmed` or `direct-permission-confirmed` attestation. |
| Discover Uni dataset | [HESA Discover Uni dataset](https://www.hesa.ac.uk/support/tools-and-downloads/unistats) | CC BY 4.0: reuse is allowed with credit, licence link, and indication of changes. HESA advises onward users to contact it about presentation considerations. Provider pages remain authoritative for application facts and requirements. | “HESA, www.hesa.ac.uk”, CC BY 4.0, with a link and statement of changes. | Weekly Wednesday check; imported courses remain drafts until provider-source review. | **Blocked** until an admin records an active `open-licence-confirmed` or `direct-permission-confirmed` attestation; this does not substitute for provider verification. |
...
The review queue at `/admin/catalogue` supports source, route type, sector, publication, freshness, open state, missing requirement, unclassified, pending revision, conflict/source issue, missing verification, and launch-failure filtering. Each readiness cell distinguishes published records from unpublished candidates and links directly to that cell's draft queue; apprenticeship shortfalls default to deadline order. Reviewers use official application and requirement-source links, compare current and proposed values, enter a note, and accept, reject, supersede, or withdraw revisions. Opportunity and requirement edits preserve previous reviewed facts in the restricted audit history.

Publication is one service-role-only transaction. It checks admin authentication at the API boundary and then fails closed in Postgres for a missing active source permission attestation, non-launch or unclassified sector, closed/unknown state, missing official facts, verification older than 30 days, expired freshness, passed deadline, pending revision, unresolved source issue, missing reviewed requirement, missing supporting evidence, conflict, unsupported deterministic rule, missing Discover Uni attribution, or a university record not manually confirmed for the 2027 cycle. The public read policy repeats these gates, so a stale or newly expired row is hidden even before the next review action. The publication state, review record, and audit event succeed together. Anonymous and ordinary authenticated clients have no execute grant. Withdrawal remains immediately available with a reviewer note.

The readiness report checks record-level publication failures plus the 80-record total, all eight 10-record cells, both 40-record route totals, diversity, duplicates, source-run health, review backlog, and three relevant open, source-backed opportunities for every promoted sector × route persona. It reports the next useful queue without exposing raw snapshots. The scheduled maintenance function also closes every non-closed opportunity whose displayed deadline has passed.
...
1. In `/admin/catalogue`, record an active administrator permission attestation for the apprenticeship API and Discover Uni. Do not treat a credential as permission; no document upload or written reference is required.
2. Configure source credentials only in the intended server environment and apply every migration through `20260827202316`.
...
6. For every requirement, preserve supporting wording, source URL, retrieval time, verification time, reviewer note, and a supported structured rule only when it is a deterministic A-level or GCSE grade rule. Do not promote a hard requirement from unstructured text by inference: select the supported qualification type, subject, and grade rule only after checking the source wording.
...
10. Do not open launch until `/api/admin/catalogue/readiness` returns `ready: true`, every promoted persona coverage row passes, and the separate security, policy, accessibility, payment, restore, and production gates pass.
...
`pnpm verify:local` runs the production dependency audit and ordinary non-destructive local/CI-equivalent checks. `pnpm release:verify` is strict and requires the exact release acknowledgement plus explicit `1` flags for the isolated Supabase attack, authenticated commercial Playwright, Stripe staging, and restore suites. A missing selection is reported as `required-but-skipped` and fails the run. Each suite prints a concise human status and the orchestrator emits a credential-free JSON summary.

### Liveness, readiness, and payment opening

- `/api/health` returns only process liveness. A `200` there does not authorise traffic, catalogue promotion, account deletion, or payments.
- `/api/readiness` returns `200` only when the canonical production origin, Supabase connection, exact `20260827202316` probe, catalogue release gate, separately durable deletion ledger, and payment-switch state are safe. It otherwise returns `503` with coarse check names and no credentials or detailed provider errors.
- Leave `PAYMENTS_ENABLED=false` for beta and while any paid-launch gate is incomplete. This blocks checkout before profile reads or inventory reservation; Free remains available. Signed Stripe webhooks remain enabled so already-created payments, refunds, and disputes can still project safely.
- To open checkout after every Gate 1 approval and isolated test passes, configure matching Stripe credentials and `STRIPE_EXPECTED_LIVEMODE`, verify `/api/readiness`, then change only `PAYMENTS_ENABLED=true` and redeploy. Turn it back to `false` to stop new checkout without interrupting lifecycle webhooks.
...
Before enabling a live Stripe key, apply every payment migration to the isolated staging Supabase project and run the opt-in payment integration suite with the locally started test-mode application configured with `PAYMENTS_ENABLED=true`, plus `PAYMENT_STAGING_BASE_URL`, `PAYMENT_STAGING_SUPABASE_URL`, `PAYMENT_STAGING_SUPABASE_SERVICE_ROLE_KEY`, `PAYMENT_STAGING_STRIPE_WEBHOOK_SECRET`, and `PAYMENT_STAGING_CRON_SECRET`. Confirm a Stripe test-mode signed webhook for each of: one paid completion, duplicate concurrent delivery, an unpaid/incomplete completion, wrong currency/amount/metadata, full refund, partial refund, dispute creation, dispute closure, an older completion after a terminal event, and a temporary database failure followed by retry. Also confirm that the protected expiry endpoint changes an ended Cycle entitlement to `expired`. Record the migration versions, event IDs (redacted where required), resulting entitlement/order/event states, and cleanup result. Do not use live customer data or a production project for this procedure.
