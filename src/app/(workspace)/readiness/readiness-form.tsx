...
type ExistingProfile = {
  profile?: {
    ...
    qualifications_complete?: boolean;
  } | null;
  qualifications?: Array<{ id: string; qualification_type: string; subject: string; grade?: string; status: "predicted" | "achieved" | "unknown" }>;
};
...
type QualificationRow = {
  ...
  subject: string;
  grade: string;
  status: "predicted" | "achieved" | "unknown";
};

function blankQualification(): QualificationRow {
  return { qualificationType: "A level", subject: "", grade: "", status: "predicted" };
}
...
export function ReadinessForm() {
  ...
  const [qualificationsComplete, setQualificationsComplete] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/profile")
      ...
      .then((data: ExistingProfile) => {
        ...
        setQualifications((data.qualifications ?? []).map((qualification) => ({
          id: qualification.id,
        ...
        setQualificationsComplete(data.profile ? (data.profile.qualifications_complete ?? false) : null);
      })
      .catch(() => setExisting({}));
  }, []);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      qualifications: qualifications.map((qualification) => ({
        ...
        subject: qualification.subject,
        ...(qualification.status === "unknown" ? {} : { grade: qualification.grade }),
        status: qualification.status,
      })),
      qualificationsComplete: qualificationsComplete ?? false,
  ...
  const updateQualification = (index: number, update: Partial<QualificationRow>) => {
    setQualifications((current) => current.map((qualification, rowIndex) => rowIndex === index ? { ...qualification, ...update } : qualification));
  };
  ...
  return (
    <form onSubmit={submit} className="space-y-6">
      ...
      <section className="space-y-5 rounded-[2rem] bg-white/80 p-5 shadow-sm sm:p-7">
        <div>
          <h2 className="text-xl font-black">Qualifications</h2>
          <p className="mt-2 text-sm font-semibold leading-6 text-ink/60">Each qualification is kept as a separate record. Routefinder can currently compare A-level and GCSE requirements deterministically; other types are saved but will need direct checking.</p>
        </div>
        <fieldset className="space-y-3">
          <legend className="font-black">Have you entered every qualification you want Routefinder to use for comparisons?</legend>
          <p className="text-sm font-semibold leading-6 text-ink/60">If not, a qualification that is not entered stays unknown rather than being treated as absent.</p>
          <label className="flex items-start gap-3 rounded-2xl border border-ink/10 bg-oat/60 p-4 font-semibold"><input required type="radio" name="qualificationsComplete" checked={qualificationsComplete === true} onChange={() => setQualificationsComplete(true)} className="mt-1 h-5 w-5 accent-leaf" /> <span>Yes, this record is complete for now.</span></label>
          <label className="flex items-start gap-3 rounded-2xl border border-ink/10 bg-oat/60 p-4 font-semibold"><input required type="radio" name="qualificationsComplete" checked={qualificationsComplete === false} onChange={() => setQualificationsComplete(false)} className="mt-1 h-5 w-5 accent-leaf" /> <span>No, I will add more qualifications later.</span></label>
        </fieldset>
        <div className="space-y-4">
          {qualifications.map((qualification, index) => {
            const rowId = qualification.id ?? `new-${index}`;
            const unknown = qualification.status === "unknown";
            const unsupported = qualification.qualificationType.trim() && !["a level", "gcse"].includes(qualification.qualificationType.trim().toLowerCase());
            return <fieldset key={rowId} className="rounded-2xl border border-ink/10 bg-oat/50 p-4">
              <legend className="px-1 font-black">Qualification {index + 1}</legend>
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="font-bold">Qualification type
                  <input required value={qualification.qualificationType} onChange={(event) => updateQualification(index, { qualificationType: event.target.value })} placeholder="A level, GCSE, BTEC…" className={field} />
                </label>
                <label className="font-bold">Subject
                  <input required value={qualification.subject} onChange={(event) => updateQualification(index, { subject: event.target.value })} placeholder="For example: Mathematics" className={field} />
                </label>
                <label className="font-bold">Result status
                  <select value={qualification.status} onChange={(event) => updateQualification(index, { status: event.target.value as QualificationRow["status"], ...(event.target.value === "unknown" ? { grade: "" } : {}) })} className={field}>
                    <option value="predicted">Predicted</option><option value="achieved">Achieved</option><option value="unknown">Unknown</option>
                  </select>
                </label>
                <label className="font-bold">Grade
                  <input required={!unknown} disabled={unknown} value={qualification.grade} onChange={(event) => updateQualification(index, { grade: event.target.value })} placeholder={unknown ? "Not known yet" : "For example: A or 6"} className={`${field} disabled:cursor-not-allowed disabled:bg-ink/5`} />
                </label>
              </div>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm font-semibold">
                <p className={unsupported ? "text-coral" : "text-ink/55"}>{unsupported ? "This qualification type is recorded, but Routefinder cannot compare it deterministically yet. Check relevant requirements directly." : "This type is within Routefinder’s current deterministic comparison vocabulary."}</p>
                <button type="button" onClick={() => setQualifications((current) => current.filter((_, rowIndex) => rowIndex !== index))} className="min-h-11 rounded-full border border-ink/15 px-4 font-black">Remove</button>
              </div>
            </fieldset>;
          })}
          {!qualifications.length && <p className="rounded-2xl bg-sky p-4 text-sm font-semibold">No qualifications are recorded yet. You can save this as incomplete, or state that the record is complete if that accurately describes your information.</p>}
          <button type="button" onClick={() => setQualifications((current) => [...current, blankQualification()])} className="min-h-12 rounded-full border border-leaf px-5 font-black text-leaf">Add qualification</button>
        </div>
      ...
      <div className="flex flex-col items-start justify-between gap-4 rounded-[2rem] bg-ink p-5 text-white sm:flex-row sm:items-center sm:p-7">
        ...
        <button disabled={saving || selectedSectors.length === 0 || qualificationsComplete === null} className="min-h-12 rounded-full bg-white px-6 font-black text-ink disabled:cursor-not-allowed disabled:opacity-50">
