...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (parsed.data.status === "completed") {
    ...
    if ((scheduledResult.count ?? 0) < 3 && profileResult.data?.application_cycle === launchApplicationCycle && !qualificationsResult.error && !portfolioResult.error && !linksResult.error && !evidenceResult.error) {
      ...
      const replacement = selectThisWeek(applySharedOpportunityCounts(candidates))[0];
      if (replacement) {
        await context.admin.from("tasks").insert({
          user_id: context.user.id,
          portfolio_item_id: replacement.portfolioItemId,
          requirement_id: replacement.requirement?.id,
          title: replacement.title,
          why_it_matters: replacement.sharedOpportunityCount && replacement.sharedOpportunityCount > 1
            ? `${replacement.whyItMatters} This gap is shared by ${replacement.sharedOpportunityCount} saved opportunities.`
            : replacement.whyItMatters,
