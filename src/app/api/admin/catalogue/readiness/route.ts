...
export async function GET() {
  ...
  const [opportunities, runs, attestations] = await Promise.all([
    ...
    admin.from("catalogue_source_attestations").select("id,source_authority,permission_basis,attested_by,attested_at,revoked_at").is("revoked_at", null).order("attested_at", { ascending: false }),
  ]);
  if (opportunities.error || runs.error || attestations.error) return apiError("Catalogue readiness is unavailable.", 503, "unavailable");
  return NextResponse.json({
    ...evaluateCatalogueReadiness(opportunities.data ?? [], runs.data ?? [], new Date(), attestations.data ?? []),
    sourceAttestations: attestations.data ?? [],
  });
}
